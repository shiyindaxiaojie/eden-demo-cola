...
(
	...
	`activated`          bit(1)      NOT NULL DEFAULT 1 COMMENT '是否激活账户（1：激活，0：不激活）',
	`locked`             bit(1)      NOT NULL DEFAULT 0 COMMENT '是否锁定账户（1：锁定，0：非锁定）',
	...
	`reset_date`         timestamp            DEFAULT NULL COMMENT '重置密码的时间',
	`created_by`         varchar(20)          DEFAULT NULL COMMENT '创建的账户',
	`created_date`       timestamp            DEFAULT current_timestamp() COMMENT '创建的时间',
	`last_modified_by`   varchar(20)          DEFAULT NULL COMMENT '最后修改的账户',
	`last_modified_date` timestamp            DEFAULT NULL COMMENT '最后修改的时间',
	PRIMARY KEY (`id`),
